...
class SettingsStorageService {
  ...
  static Map<String, dynamic>? loadTheme() {
    final json = _prefs?.getString(_keyTheme);
  ...
  static Future<void> saveTheme(Map<String, dynamic> data) async {
    await _prefs?.setString(_keyTheme, jsonEncode(data));
  }

  // ─── AUDIO SETTINGS ───

  static Map<String, dynamic>? loadAudioSettings() {
    final json = _prefs?.getString(_keyAudio);
    if (json == null) return null;
    try {
      return jsonDecode(json) as Map<String, dynamic>;
    } catch (_) {
  ...
  static Future<void> saveAudioSettings(Map<String, dynamic> data) async {
    await _prefs?.setString(_keyAudio, jsonEncode(data));
  }

  // ─── LIBRARY SETTINGS ───

  static Map<String, dynamic>? loadLibrarySettings() {
    final json = _prefs?.getString(_keyLibrary);
    if (json == null) return null;
    try {
      return jsonDecode(json) as Map<String, dynamic>;
    } catch (_) {
  ...
  static Future<void> saveLibrarySettings(Map<String, dynamic> data) async {
    await _prefs?.setString(_keyLibrary, jsonEncode(data));
  }
  ...
  static Map<String, dynamic>? loadLibraryCache() {
    final json = _prefs?.getString(_keyLibraryCache);
    if (json == null) return null;
    try {
      return jsonDecode(json) as Map<String, dynamic>;
    } catch (_) {
  ...
  static Future<void> saveLibraryCache(Map<String, dynamic> data) async {
    await _prefs?.setString(_keyLibraryCache, jsonEncode(data));
  }

  static bool get hasLibraryCache => _prefs?.containsKey(_keyLibraryCache) ?? false;

  // ─── MUSIC FOLDERS ───

  /// Carrega pastas de música selecionadas. Null = nunca configurou (1ª vez).
  static List<String>? loadMusicFolders() {
    final list = _prefs?.getStringList(_keyMusicFolders);
    return list;
  }

  /// Salva pastas de música selecionadas.
  static Future<void> saveMusicFolders(List<String> folders) async {
    await _prefs?.setStringList(_keyMusicFolders, folders);
  }

  /// Verifica se o usuário já configurou pastas.
  static bool get hasMusicFolders => _prefs?.containsKey(_keyMusicFolders) ?? false;

  // ─── USER PLAYLISTS ───

  static String? loadUserPlaylists() {
    return _prefs?.getString(_keyUserPlaylists);
  }

  static Future<void> saveUserPlaylists(String jsonString) async {
    await _prefs?.setString(_keyUserPlaylists, jsonString);
  }
