...
class StatisticsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final totalDuration = songs.fold<Duration>(Duration.zero, (sum, s) => sum + s.duration);
    ...
    return BackgroundWrapper(
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Estatisticas', style: theme.textTheme.titleLarge?.copyWith(
        ...
        body: ListView(
          ...
          children: [
            // -- Library Overview --
            Text('BIBLIOTECA', style: theme.textTheme.labelSmall?.copyWith(
            ...
            Wrap(
              ...
              children: [
                _StatCard(label: 'Musicas', value: '$totalSongs', icon: Icons.music_note_rounded, color: colors.primary, colors: colors, theme: theme),
                _StatCard(label: 'Albuns', value: '$totalAlbums', icon: Icons.album_rounded, color: const Color(0xFF64B5F6), colors: colors, theme: theme),
                _StatCard(label: 'Artistas', value: '$totalArtists', icon: Icons.person_rounded, color: const Color(0xFFBA68C8), colors: colors, theme: theme),
                _StatCard(label: 'Generos', value: '$totalGenres', icon: Icons.category_rounded, color: const Color(0xFFFFB74D), colors: colors, theme: theme),
                _StatCard(label: 'Favoritas', value: '$favCount', icon: Icons.favorite_rounded, color: colors.error, colors: colors, theme: theme),
                _StatCard(label: 'Duracao Total', value: '${hours}h ${mins}m', icon: Icons.timer_rounded, color: const Color(0xFF4DD0E1), colors: colors, theme: theme),
              ],
            ),
            ...
            // -- Playback Stats --
            Text('REPRODUCAO', style: theme.textTheme.labelSmall?.copyWith(
              color: colors.onSurface.withValues(alpha: 0.4),
              letterSpacing: 1.2,
            )),
            const SizedBox(height: AppSpacing.sm),
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.play_circle_rounded, size: 40, color: colors.primary),
                  const SizedBox(width: AppSpacing.md),
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('$totalPlays', style: theme.textTheme.headlineMedium?.copyWith(
                        color: colors.onSurface, fontWeight: FontWeight.w700)),
                      Text('reproducoes totais', style: theme.textTheme.bodySmall?.copyWith(
                        color: colors.onSurface.withValues(alpha: 0.4))),
                    ],
                  ),
                ],
            ...
            if (top5.isNotEmpty) ...[
              const SizedBox(height: AppSpacing.lg),
              Text('MAIS TOCADAS', style: theme.textTheme.labelSmall?.copyWith(
                color: colors.onSurface.withValues(alpha: 0.4),
                letterSpacing: 1.2,
              )),
              const SizedBox(height: AppSpacing.sm),
              ...top5.asMap().entries.map((entry) {
                ...
                return ListTile(
                  ...
                  leading: CircleAvatar(
                    ...
                    child: Text('${idx + 1}', style: theme.textTheme.labelLarge?.copyWith(
                  ...
                  subtitle: Text(song.artist, maxLines: 1, overflow: TextOverflow.ellipsis,
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: colors.onSurface.withValues(alpha: 0.4))),
                  trailing: Text('${count}x', style: theme.textTheme.labelLarge?.copyWith(
            ...
            if (topGenres.isNotEmpty) ...[
              const SizedBox(height: AppSpacing.lg),
              Text('GENEROS MAIS OUVIDOS', style: theme.textTheme.labelSmall?.copyWith(
                color: colors.onSurface.withValues(alpha: 0.4),
                letterSpacing: 1.2,
              )),
              const SizedBox(height: AppSpacing.sm),
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Text('${e.value} musicas', style: theme.textTheme.bodySmall?.copyWith(
                            color: colors.onSurface.withValues(alpha: 0.4))),
                        ],
                      ),
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final w = (MediaQuery.sizeOf(context).width - AppSpacing.md * 2 - AppSpacing.sm) / 2;
    return Container(
      width: w,
      padding: const EdgeInsets.all(AppSpacing.md),
      decoration: BoxDecoration(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(label, style: theme.textTheme.bodySmall?.copyWith(
                  color: colors.onSurface.withValues(alpha: 0.4), fontSize: 10)),
              ],
            ),
          ),
        ],
